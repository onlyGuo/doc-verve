...
public class GenericsUtils {
	 ...
	 * 		泛型参数所在索引,从0开始.
	 * @return
	 * 		范型参数的实际类型, 如果没有实现ParameterizedType接口，即不支持泛型，所以直接返回
	 ...
	 * @return
	 * 		泛型参数的实际类型, 如果没有实现ParameterizedType接口，即不支持泛型，所以直接返回
	 *      <code>Object.class</code>
	 */
	...
	public static Class getMethodGenericReturnType(Method method, int index) {
		...
		return getClassGenericType(returnType, index);
	}

	private static Class getClassGenericType(Type paramType, int index){
	...
	public static Class getFieldGenericType(Field field, int index) {
		...
		return getClassGenericType(genericFieldType, index);
	}
