package com.guoshengkai.doc.core.sql.where;

import com.alibaba.fastjson.JSON;
import com.guoshengkai.doc.core.annotation.po.*;
import com.guoshengkai.doc.core.beans.PO;
import com.guoshengkai.doc.core.beans.Pram;
	...
	public static boolean setFileValue(PO po, String fileName, Serializable fileValue){
        ...
        try {
			...
			if (typeName.equals("int") || typeName.equals(Integer.class.getName())){
				...
				writeMethod.invoke(po, v);
			} else if (fileValue instanceof LocalDateTime && typeName.equals("java.util.Date")) {
				writeMethod.invoke(po, Date.from(((LocalDateTime) fileValue)
						.atZone(ZoneId.systemDefault()).toInstant()));
			}else{
				try {
					if(Class.forName(typeName).isEnum()){
						writeMethod.invoke(po, Enum.valueOf((Class<Enum>)propertyDescriptor.getPropertyType(), fileValue.toString()));
					}else{
