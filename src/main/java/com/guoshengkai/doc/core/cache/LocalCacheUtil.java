...
public class LocalCacheUtil {
    ...
    public static void put(Key key, Object value, TimeUnit timeUnit, long time){
        ...
        LocalCacheUtil.cacheItemMap.put(key.toString(), item);
    }
    ...
    public static <T> T lock(long time, Key key, LockRunner<T> runner){
        ...
        while (null != integer){
            if (System.currentTimeMillis() - l > time){
                throw new ServiceInvokeException("业务锁超时");
            }
            try {
                Thread.sleep(100);
            } catch (InterruptedException e) {
